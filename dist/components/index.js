...
var mobileToc_inline_default = 'function l(){let i=document.querySelectorAll("div.notes-toc-explorer");for(let e of i){e.classList.remove("notes-toc-open"),e.classList.remove("collapsed"),e.setAttribute("aria-expanded","true"),document.documentElement.classList.remove("mobile-no-scroll");let o=[],r=e.querySelector(".mobile-toc-explorer");if(r){r.classList.remove("hide-until-loaded"),requestAnimationFrame(()=>{requestAnimationFrame(()=>e.classList.add("notes-toc-ready"))});let t=()=>{let s=e.classList.toggle("notes-toc-open");e.setAttribute("aria-expanded",s?"true":"false"),s?document.documentElement.classList.add("mobile-no-scroll"):document.documentElement.classList.remove("mobile-no-scroll")};r.addEventListener("click",t),o.push(()=>r.removeEventListener("click",t))}let n=e.querySelector(".desktop-toc-explorer");if(n){let t=()=>{let s=e.classList.toggle("collapsed");e.setAttribute("aria-expanded",s?"false":"true")};n.addEventListener("click",t),o.push(()=>n.removeEventListener("click",t))}for(let t of e.querySelectorAll("a")){let s=()=>{window.matchMedia("(max-width: 800px)").matches&&(e.classList.remove("notes-toc-open"),e.setAttribute("aria-expanded","false"),document.documentElement.classList.remove("mobile-no-scroll"))};t.addEventListener("click",s),o.push(()=>t.removeEventListener("click",s))}window.addCleanup&&window.addCleanup(()=>o.forEach(t=>t()))}window.__notesTocObserver?.disconnect();let c=new IntersectionObserver(e=>{for(let o of e){let r=o.target.id,n=document.querySelectorAll(`a[data-for="${r}"]`);n.length&&(o.isIntersecting?n.forEach(t=>t.classList.add("in-view")):n.forEach(t=>t.classList.remove("in-view")))}},{rootMargin:"20px 0px 0px 0px"});window.__notesTocObserver=c,document.querySelectorAll("h1[id], h2[id], h3[id], h4[id], h5[id], h6[id]").forEach(e=>c.observe(e))}document.addEventListener("nav",l);document.addEventListener("render",l);\n';
var l;
...
export { Backlinks_default as Backlinks, MobileToc_default as MobileToc };
